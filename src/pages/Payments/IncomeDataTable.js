...
const IncomeDataTable = ({ id, getAPIData, strAPIName }) => {
  ...
  const handleSearch = () => {
    var filteredRows = initialRows.filter((row) => {
      console.log(new Date(formData.from), new Date(formData.to))
      const matchesFrom =
        formData.from ? new Date(row.date) >= new Date(formData.from) : true;
      const matchesTo =
        formData.to ? new Date(row.date) <= new Date(formData.to) : true;
  ...
  const columns = [
    ...
    {
      field: "category", headerName: "Category", width: 180, type: "singleSelect", editable: true,
      valueOptions: ["Fixed Amount", "Per session"]
    },
    {
      field: "subcategory", headerName: "Subcategory", width: 180, type: "singleSelect", editable: true,
      valueOptions: ["Fixed Amount", "Per session"]
    },
